  ...
  return (
    <>
      ...
      <button
        ...
        className="group rounded-xl border-2 border-dashed border-stone-300 text-stone-400 aspect-square flex flex-col items-center justify-center gap-2 hover:bg-stone-50 hover:text-stone-600 hover:border-stone-400 active:scale-[0.98] transition-all disabled:opacity-50"
      >
        {uploading ? (
...
function InspoTile({ inspo, onDeleted }: { inspo: InspoWithUrl; onDeleted: () => void }) {
  ...
  return (
    <div className={`relative aspect-square rounded-xl overflow-hidden bg-stone-100 group ${deleting ? 'opacity-50 pointer-events-none' : ''}`}>
      <Image
        src={inspo.url}
      ...
      <button
        ...
        className="absolute top-1.5 right-1.5 bg-black/60 text-white rounded-full w-7 h-7 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
      >
...
export default function InspoGrid({ inspos }: { inspos: InspoWithUrl[] }) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
      ...
      {inspos.map((inspo) => (
        <InspoTile key={inspo.id} inspo={inspo} onDeleted={refresh} />
      ))}
    </div>
  );
}
